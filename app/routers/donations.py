...
async def claim_donation(
    ...
    if donation.status != "available":
        raise HTTPException(status_code=400, detail=f"Too late! This donation is already {donation.status}")

    # Ensure ONLY ngos/volunteers can claim food (Donors cannot claim their own food)
    if current_user.role == "donor":
        raise HTTPException(status_code=403, detail="Donors cannot claim food donations.")
